...
const computeDerivedFields = (ticket) => {
  ...
  const slaBreached = ageMinutes > slaTarget || 
    (ticket.status === 'resolved' && ageMinutes > slaTarget);
...
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/deskflow')
      ...
      if (req.body.status && req.body.status !== ticket.status) {
        if (!validateTransition(ticket.status, req.body.status)) {
          return res.status(400).json({ 
            error: `Cannot transition from ${ticket.status} to ${req.body.status}` 
          });
        }
